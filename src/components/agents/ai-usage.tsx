      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <CardDescription>
              Tokens consumidos na chave do seu provedor pelas sugestões e
              respostas automáticas. O conteúdo das mensagens não é armazenado aqui.
            </CardDescription>
          </div>
          <Select
            value={String(days)}
      ...
      <CardContent className="space-y-5">
        {loading || !data ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center text-sm text-muted-foreground">
            ...
            <p>Ainda não houve uso de IA nos últimos {data.window_days} dias.</p>
            <p className="text-xs">
              Os dados aparecerão conforme o assistente sugerir e enviar respostas automáticas.
            </p>
          </div>
        ) : (
          <>
                  ...
                  {data.by_model.map((m) => (
                    ...
                    >
                      ...
                      <span className="flex-shrink-0 tabular-nums text-muted-foreground">
                        {formatCompactNumber(m.tokens)} tokens · {m.calls}{' '}
            ...
            {data.truncated && (
              <p className="text-xs text-muted-foreground">
                Exibindo um período parcial — como o uso é alto, somente os
