...
export function MessageComposer({
    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === "ai_not_configured") {
          ...
          toast.error(data.error ?? "Não foi possível sugerir uma resposta.");
        }
        return;
      }
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      if (file.size > max) {
        toast.error(
          `O arquivo possui ${(file.size / 1024 / 1024).toFixed(1)} MB — o limite para este tipo é ${Math.round(
            max / 1024 / 1024,
          )} MB.`,
        );
        return;
  ...
  const finalizeRecording = useCallback(
    async (bytes: Uint8Array) => {
      ...
      if (file.size > MEDIA_MAX_BYTES_BY_KIND.audio) {
        toast.error("A gravação é muito longa e ultrapassou 16 MB.");
        return;
      }
      setBusy(true);
      try {
        ...
        toast.error(err instanceof Error ? err.message : "Falha no envio da gravação.");
      } finally {
        setBusy(false);
      }
    },
  ...
  const startRecording = useCallback(async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof AudioContext === "undefined") {
      toast.error("Este navegador não permite gravar mensagens de voz.");
      return;
    }
    try {
