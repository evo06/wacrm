...
describe("validateFlowForActivation — flow-level", () => {
  ...
  it("flags empty node list", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("pelo menos um bloco")),
    ).toBe(true);
  });
  ...
  it("flags duplicate node_key", () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.message.includes("está duplicada") &&
          i.node_key === "a",
      ),
    ).toBe(true);
  });
  ...
  it("flags keyword trigger with no keywords", () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.scope === "trigger" &&
          i.message.includes("pelo menos uma palavra"),
      ),
    ).toBe(true);
  });
...
describe("validateFlowForActivation — nodes", () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "b" &&
          i.field === "buttons" &&
          i.message.includes("pelo menos um botão"),
      ),
    ).toBe(true);
  });
    ...
    expect(
      issues.some(
        (i) =>
          ...
          i.message.includes("ultrapassa 20"),
      ),
    ).toBe(true);
  });
  ...
  it("flags duplicate button reply_ids", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("ID de resposta") && i.message.includes("duplicado")),
    ).toBe(true);
  });
  ...
  it("flags list row title over 24 chars", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("ultrapassa 24 caracteres")),
    ).toBe(true);
  });
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "orphan" &&
          i.severity === "warning" &&
          i.message.includes("não pode ser alcançado"),
      ),
    ).toBe(true);
  });
  ...
  it("doesn't crash on unknown node_type — flags it", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("Tipo de bloco desconhecido")),
    ).toBe(true);
  });
});
